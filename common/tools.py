...
def list_directory_contents(path_to_list: str, base_dir_context: str, 
                          include_hidden: bool = False, tool_context: ToolContext | None = None) -> Dict[str, Any]:
        ...
        for entry in os.scandir(resolved_path):
            ...
            try:
                ...
                info = {
                    ...
                    "path": entry.path,
                    "size": stats.st_size,
                    "modified": datetime.datetime.fromtimestamp(stats.st_mtime, tz=ZoneInfo("UTC")).isoformat(),
...
def read_file_content(file_path_to_read: str, base_dir_context: str,
...
def get_dependencies(target_directory: str, tool_context: ToolContext | None = None) -> Dict[str, Any]:
            ...
            if os.path.exists(file_path):
                if filename == "requirements.txt":
                    with open(file_path, 'r') as f:
                        dependencies[key] = [line.strip() for line in f 
    ...
    return {"dependencies": dependencies, "path_checked": target_directory} if dependencies else \
           {"message": "No dependency files found", "path_checked": target_directory}
...
def search_codebase(target_directory: str, keywords: str, file_pattern: str = "*.*",
                   context_lines: int = 15, ignore_case: bool = True,
                   tool_context: ToolContext | None = None) -> Dict[str, Any]:
    """Search codebase for keywords with smart directory filtering."""
    
    # Directories to skip (consolidated from original verbose lists)
    SKIP_DIRS = {
        'node_modules', 'target', 'build', 'dist', 'out', 'venv', '.venv', 'env', '.env',
        'migrations', 'bin', 'obj', 'logs', 'temp', 'tmp', '__pycache__', '.pytest_cache',
        '.mypy_cache', '.cache', '.tox', 'site-packages', 'vendor'
    }
    SKIP_PREFIXES = ('.', '__')
    ...
    try:
        ...
        for root, dirs, files in os.walk(target_directory, topdown=True):
            ...
            dirs[:] = [d for d in dirs if not (
                d.startswith(SKIP_PREFIXES) or d.lower() in SKIP_DIRS
            )]
...
def search_code_with_prompt(target_directory: str, prompt_text: str, 
                          file_pattern: str = "*.*", tool_context: ToolContext | None = None) -> Dict[str, Any]:
    """Search code using natural language prompt as keywords."""
    if not prompt_text.strip():
        return {"error": "Prompt text cannot be empty"}
    
    return search_codebase(target_directory, prompt_text, file_pattern, 
                          context_lines=15, ignore_case=True, tool_context=tool_context)

def search_tests_with_prompt(target_directory: str, prompt_text: str, file_pattern: str,
                           tool_context: ToolContext | None = None) -> Dict[str, Any]:
...
def determine_relevance_from_prompt(prompt_text: str, found_files_context: List[Dict[str, Any]],
                                  tool_context: ToolContext | None = None) -> Dict[str, Any]:
...
def get_session_state(key: str, default: Optional[Any] = None, 
                     tool_context: ToolContext | None = None) -> Any:
