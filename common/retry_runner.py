    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt >= max_retries:
                if log:
                    log.error(f"Max retries ({max_retries}) exceeded. Last error: {error_content[:100]}...")
                break
            ...
            if is_429:
                ...
                if log:
                    log.warning(f"429 rate limit. Retrying in {delay:.1f}s. Error: {error_content[:100]}...")
            else:
                ...
                if log:
                    log.warning(f"Error on attempt {attempt + 1}. Retrying in {delay:.1f}s. Error: {error_content[:100]}...")
