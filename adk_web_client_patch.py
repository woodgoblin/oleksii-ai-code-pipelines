    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            for attempt in range(max_retries + 1):
                try:
                    logger.info(
                        f"🤖 Google AI call attempt {attempt + 1}/{max_retries + 1}: {func.__name__}"
                    )
                    ...
                    if "MALFORMED_FUNCTION_CALL" in error_str or "finish_reason" in error_str:
                        if attempt >= max_retries:
                            logger.error(
                                f"💀 Max retries ({max_retries}) exceeded for {func.__name__} (MALFORMED_FUNCTION_CALL)"
                            )
                            raise

                        delay = base_delay * (2**attempt) + random.uniform(0, 1)
                        logger.warning(
                            f"🔧 MALFORMED_FUNCTION_CALL: Retrying {func.__name__} in {delay:.1f}s (attempt {attempt + 2})"
                        )
                    ...
                    # Check if it's a 429 error
                        ...
                        if retry_delay:
                            ...
                            logger.warning(
                                f"⏳ 429 Error: Waiting {delay}s (Google AI suggested {retry_delay}s + 1s buffer)"
                            )
                        else:
                            delay = base_delay * (2**attempt) + random.uniform(0, 1)
                            logger.warning(
                                f"⏳ 429 Error: Waiting {delay:.1f}s (exponential backoff)"
                            )
                    ...
                    # Check if it's another retryable error (5xx)
                        ...
                        delay = base_delay * (2**attempt) + random.uniform(0, 1)
                        logger.warning(
                            f"⏳ Server Error: Waiting {delay:.1f}s before retry {attempt + 2}"
                        )
                        await asyncio.sleep(delay)
                        continue
                    else:
...
def patch_google_ai_client():
    ...
    try:
        ...
        if hasattr(AsyncModels, "generate_content"):
            ...
            patched_generate_content = google_ai_retry_decorator(max_retries=30, base_delay=2.0)(
        ...
        try:
            ...
            if hasattr(AsyncAPIClient, "_async_request_once"):
                ...
                patched_async_request = google_ai_retry_decorator(max_retries=30, base_delay=2.0)(
                    original_async_request
                )
        ...
        logger.info("   • MALFORMED_FUNCTION_CALL retry with exponential backoff")
